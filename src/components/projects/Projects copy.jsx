    ...
    <div className=" bg-slate-900 sd:w-1/3 ">
      ...
      <div className="grid auto-rows-[192px] grid-cols-3 gap-4 gap-y-5">
        {[...Array(7)].map((_, i) => (
          <div
            key={i}
            className={`flex row-span-1 rounded-xl border-2 hover:bg-black dark:backdrop-blur-sm dark:bg-white/10 border-slate-400/10 bg-neutral-100 p-4 dark:bg-gray-800  shadow-md dark:shadow-cyan-300 ${
              i === 3 || i === 6 ? "col-span-2" : ""
            }`}
          >
                ...
                return (
                  <div className="flex flex-col h-max" key={item.id}>
                    <h3 className="text-cyan-400 font-bold">
                      {item.name}{" "}
                      <span className="text-xs text-yellow-800 bg-yellow-300 rounded pr-1 pl-1">
                        {item.version}
                      </span>
                    </h3>
                    <p className="font-semibold text-green-400 border-b-2 border-t-2">
                      {item.description}
                    </p>
                    <div className="flex flex-wrap">
                      {item.tecnologias.map((item, ind) => (
                        <span
                          className="text-sky-400 font-semibold rounded"
                          key={ind}
                        >
                          {"["}
                          {item}
